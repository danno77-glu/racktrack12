    ...
    export const useAuditForm = () => {
      ...
      const handleSubmit = async (e) => {
        ...
        try {
          ...
          // If this was a scheduled audit, mark it as completed
          if (location.state?.auditData) {
            const { auditData: scheduledAudit } = location.state;
      
            // Mark the scheduled audit as completed
            const { error: updateError } = await supabase
              .from('scheduled_audits')
              .update({ completed: true })
              .eq('id', scheduledAudit.scheduled_audit_id)
              .single(); // Ensure only one record is updated
      
            if (updateError) {
              console.error('Error updating scheduled audit:', updateError);
              throw updateError;
            }
